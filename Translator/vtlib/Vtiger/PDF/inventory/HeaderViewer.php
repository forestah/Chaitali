...
class Vtiger_PDF_InventoryHeaderViewer extends Vtiger_PDF_HeaderViewer {
	...
	function display($parent) {
		...
		if($this->model) {
			...
			//echo $modelColumn0['logo']."----".$headerFrame->x."----".$headerFrame->y."-----".$imageWidth."----".$imageHeight;die;
			...
			if (strpos($this->model->get('title'), 'Invoice:') !== false) {
				$inv_title = split('Invoice:',$this->model->get('title'));
				$contentWidth = $pdf->GetStringWidth("Invoice number: ".$inv_title[1]);
			}else{
			...
			if (strpos($this->model->get('title'), 'Invoice:') !== false) {
			$inv_title = split('Invoice:',$this->model->get('title'));
    $pdf->MultiCell($contentWidth*2.0, $contentHeight, 'Invoice number: '.$inv_title[1], 0, 'R', 0, 1, $contentX-$contentWidth,
				 $headerFrame->y+2);
				 }else{
			...
			foreach($modelColumn2 as $label => $value) {
				...
				$pdf->SetFillColor(205,201,201);
					...
					if($label=='Invoice Address' || $label=='Delivery Address' || $label=='Billing Address'){
						$offsetY = 17;
					...
					/*	$width1=$pdf->GetStringWidth($value);
$height1=$pdf->GetStringHeight($value,$width1);
$pdf->MultiCell($headerColumnWidth-$offsetX, 7, $label, 1, 'L', 1, 1, $headerFrame->x+$headerColumnWidth*2.0+$offsetX,
$pdf->GetY()+$offsetY-$height1-$offsetX-4.0);
...
$pdf->GetY());*/
						...
						$pdf->MultiCell($headerColumnWidth-$offsetX, 7, $label, 1, 'L', 1, 1, $headerFrame->x+$headerColumnWidth*2.0+$offsetX, 
										...
										$pdf->SetFont('freeserif', ''); 
										$pdf->MultiCell($headerColumnWidth-$offsetX, 7, $value, 1, 'L', 0, 1, $headerFrame->x+$headerColumnWidth*2.0+$offsetX,$pdf->GetY()); 
