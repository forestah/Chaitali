...
Vtiger.Class("Vtiger_Detail_Js",{
	...
	*/
	...
	if((app.getModuleName() == 'PurchaseOrder') && (data['field'] == "postatus") && (data['value'] != "Approved") && (is_admin != "on") && (data['value'] != "Delivered") ){
		//alert(data['value']+"    "+data['field']);
		jQuery("#PurchaseOrder_detailView_moreAction_Export_To_PDF").remove();
	}
	...
	ajaxEditHandling : function(currentTdElement){
		...
		if(jQuery('.editElement',editElement).length === 0){
			...
			//alert(fieldInfo['picklistvalues']);
			if((fieldInfo.name == "postatus") && (jQuery("#is_admin").val() != "on")){
				fieldInfo['picklistvalues'] =  jQuery.parseJSON( '{ "Created":"Created" }' );
				fieldInfo['picklistColors'] =  jQuery.parseJSON( '{ "Created":null }' );
				
			}
			/*alert(fieldInfo.name);
			alert(JSON.stringify(fieldInfo.picklistvalues));
			alert(JSON.stringify(fieldInfo.picklistColors));
