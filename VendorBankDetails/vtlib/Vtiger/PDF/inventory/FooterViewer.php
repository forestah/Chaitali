...
class Vtiger_PDF_InventoryFooterViewer extends Vtiger_PDF_FooterViewer {
	...
	static $VENDORBANKDETAIL_LABEL_KEY = '__VBANK_LABEL__';
	static $VENDORBANKDETAIL_DATA_KEY = '__VBANK__DATA__';
	static $VENDORBANKDETAIL_FLAG = '0';
	...
	function display($parent) {
		...
		if($this->model) {
			...
			//echo $vid; echo $this->focusColumnValue("record_id"); echo $module ;die;
			if($this->labelModel->get(self::$VENDORBANKDETAIL_FLAG) == '1'){
			
				$vendorLabelString = $this->labelModel->get(self::$VENDORBANKDETAIL_LABEL_KEY);
			$vendorDetails = $this->model->get(self::$VENDORBANKDETAIL_DATA_KEY);
			$offsetY = 2.0;
			$vendorDetailsHeight = $pdf->GetStringHeight($vendorLabelString, $footerFrame->w);
			$pdf->SetFillColor(205,201,201);
			$pdf->MultiCell($footerFrame->w, $vendorDetailsHeight, $vendorLabelString, 1, 'L', 1, 1,
				$pdf->GetX(), $pdf->GetY() + $offsetY);
			$pdf->MultiCell($footerFrame->w, $targetFooterHeight - $vendorDetailsHeight, $vendorDetails,1, 'L', 0, 1,
				$pdf->GetX(), $pdf->GetY());
			}
		}
	}
